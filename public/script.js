  ...
  .then((data) => {
    ...
    for (let i = 0; i < data.res.length; i++) {
      ...
      dynamic.innerHTML += `<div class="fetched-data">
        ...
        <div class="last-traded-price"> <i class="fa fa-inr" aria-hidden="true"></i> ${
          result[i].last
        }</div>
        
        <div class="buy-price"> <i class="fa fa-inr" aria-hidden="true"></i> ${result[
          i
        ].buy.toFixed(2)}/${result[i].sell.toFixed(2)}</div>
        
        <div class="difference">${diff.toFixed(2)}%</div>
        
        <div class="savings"> <i class="fa fa-inr" aria-hidden="true"></i> ${(
          result[i].buy - result[i].sell
        ).toFixed(2)} </div>
